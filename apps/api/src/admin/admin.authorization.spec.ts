...
describe('Admin and super-admin authorization portfolio', () => {
  ...
  it('allows only approved organizer members into the organizer admin shell', async () => {
    const findFirst = jest
      .fn()
      .mockResolvedValueOnce({ id: 'membership-approved' })
    ...
    await expect(
      guard.canActivate(contextFor({ ...organizer, sub: 'ordinary-user' })),
  ...
  it('scopes organizer event access to approved organizations while super admin remains cross-event', async () => {
    const findFirst = jest
      .fn()
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce({ id: 'any-event' });
    ...
    await expect(access.assertEventAccess('foreign-event', organizer)).rejects.toBeInstanceOf(
      NotFoundException,
    );
  ...
  it('blocks organizer accounts from platform-wide users, organizer governance, and settings', () => {
    ...
    expect(() => controller.getExecutiveMetricDefinitions(organizer)).toThrow(
      'Platform admin access required',
    );
    expect(() => controller.getExecutiveAnalytics(organizer)).toThrow(
      'Platform admin access required',
    );
    expect(() =>
      controller.setOrganizerProfileVisibility(
        'org-1',
        { visible: true, reason: 'Routine restoration' },
        organizer,
      ),
  ...
  it('allows super admin platform governance but prevents self-removal of its own role', async () => {
    ...
    expect(() => controller.setUserRole(superAdmin.sub, { isAdmin: false }, superAdmin)).toThrow(
      BadRequestException,
    );
