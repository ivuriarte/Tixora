...
export default function ProfilePage() {
  ...
  async function handlePasswordChange(e: React.FormEvent) {
    ...
    try {
      ...
      const msg = err?.response?.data?.message;
      toast.error(Array.isArray(msg) ? msg.join(', ') : msg ?? 'Failed to update password.');
    } finally {
      setPwSaving(false);
    }
  }
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-2xl px-4 py-10">
        ...
        {/* Change Password — platform admins only */}
        {authUser?.isAdmin && (
          <form onSubmit={handlePasswordChange} className="mt-6 space-y-4 rounded-lg border border-[#e4dcf4] bg-white p-6">
            <div>
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-widest">Change Password</h2>
              <p className="text-xs text-gray-400 mt-1">Only platform admins use password authentication.</p>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">Can&apos;t remember it? Log out and use &quot;Forgot password&quot; to reset it.</p>
            </div>
            ...
            <button
              type="submit"
              disabled={pwSaving}
              className="w-full bg-primary text-white font-semibold py-2.5 rounded-xl hover:bg-primary/90 transition-colors disabled:opacity-60 text-sm"
            >
